...
class Diagnosis
  ...
  attr_accessor :date_time_clinically_recognised
  ...
  def save
    ...
    response = client.post(url, {query: params, body: flat_json, header: header})
    response
  end
  ...
  class << self
    def all
      ...
      JSON.parse(response.body)['rows'].map do |diagnosis|
        name_value = name_code = name_terminology = onset = at0003 = diagnostic_certainty = diagnostic_certainty_code = diagnostic_certainty_terminology = nil
        ...
        self.new(id: id, onset: onset, name_value: name_value, name_terminology: name_terminology, name_code: name_code,
          diagnostic_certainty: diagnostic_certainty, diagnostic_certainty_code: diagnostic_certainty_code, diagnostic_certainty_terminology: diagnostic_certainty_terminology)
      end
    end
    ...
    def parse_from_fhir(json)
      ...
      self.new(onset: hash['onsetDateTime']['value'],
          name_terminology: hash['code']['coding'][0]['system'],
          name_code: hash['code']['coding'][0]['code'],
          name_value: hash['code']['text'],
          diagnostic_certainty_terminology: diagnostic_certainty_terminology,
          diagnostic_certainty_code: diagnostic_certainty_code,
    ...
    def parse_from_mml(xml)
      ...
      unless certainty.blank?
        ...
        if certainty.text == 'confirmedDiagnosis'
          ...
          diagnostic_certainty = 'Suspected'
          diagnostic_certanty_code = 'at0074'
        end
      end
      self.new(onset: mml['mmlRd:startDate'],
